...
- `MMH3ReplaceSection` - splice a rewritten section back into a prompt, so a refinement
  ...
  the fragile half. Mistral reliably did the rewrite and returned the body alone - the
  lint then reported all six sections missing, which reads as total failure when the
  expansion was actually fine. Give the refiner one job (return the new body, no labels)
  and let this node hold the structure. Dropping a section becomes impossible.

  It also normalises what comes back: code fences, a repeated label, and markdown
  decoration are stripped, since the text encoder receives those characters literally.

### Fixed
- `MMH3PromptLint` names a DECORATED label instead of calling it missing. Instruct
  models format the prompt as a document - `**subject_definitions:**`, `### summary` -
  and every one of those counted as absent. Six baffling absences now read as six
  instances of one fixable thing, with the offending text quoted. It is still an error:
