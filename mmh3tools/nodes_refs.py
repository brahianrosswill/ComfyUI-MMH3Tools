...
class MMH3ImageKeyframe(io.ComfyNode):
    ...
    Composing with references needs core patch 1 from `docs/core-patches.md`: stock
    `extra_conds` assigns `cond_video_latents` from keyframes and then assigns it AGAIN
    from references, so the references win and every keyframe is silently dropped. That
    one is still a file edit -- `patch_layout.py` only subsumes patch 2.
    ...
    def execute(cls, conditioning, image, vae, width, height, target_frame_count,
        ...
        if index != 0 and index != int(target_frame_count) - 1:
            # This branch HAS the runtime patch, so an interior anchor is legal -- but
            # only if the patch actually took. Refuse on a failed self-test rather than
            # letting stock PackedLayout raise from deeper in, where it reads as a
            # model error instead of a missing capability.
            from .patch_layout import is_applied, status
            if not is_applied():
                raise ValueError(
                    "frame_index %d is an INTERIOR anchor, which needs the layout patch, "
                    "and that is not active (%s). Use 0 or -1 here, or fix the patch."
                    % (index, status()))
            logging.info("[MMH3ImageKeyframe] interior anchor at frame %d", index)
...
class MMH3LatentKeyframe(io.ComfyNode):
    ...
    Keyframes and references coexist here only with core patch 1 applied (see
    `docs/core-patches.md`). Stock `extra_conds` assigns `cond_video_latents` from
    keyframes and then ASSIGNS IT AGAIN from references, so any reference silently
    erases every keyframe. `patch_layout.py` subsumes patch 2, not patch 1.
